...
from app.web.utils.template import generate_new_account_email, send_email
...
async def read_user_context(session: SessionDep, current_user: CurrentUser) -> UserContext:
    """Return tenants and teams available to the current user."""
    statement = (
        select(Team, Tenant)
        .join(Tenant, Team.tenant_id == Tenant.id)
        .where(Tenant.status == StatusTypes.ENABLE)
    )

    if not current_user.is_superuser:
        memberships = (
            select(Team.tenant_id)
            .join(TeamUserJoin, TeamUserJoin.team_id == Team.id)
            .where(
        ...
        if current_user.is_tenant_admin:
            statement = statement.where(Team.tenant_id.in_(memberships))
        else:
    ...
    rows = (await session.exec(statement.order_by(Tenant.name, Team.name))).all()
    tenants = {
        tenant.id: UserContextTenant(id=tenant.id, name=tenant.name)
        for _, tenant in rows
    }
    teams = [
        UserContextTeam(id=team.id, name=team.name, tenant_id=tenant.id)
        for team, tenant in rows
    ]
    return UserContext(user=UserOut.model_validate(current_user), tenants=list(tenants.values()), teams=teams)
