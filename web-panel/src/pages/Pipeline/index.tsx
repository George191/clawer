...
function  moveFilterRows<T extends { id: string }>(rows: T[], index: number, direction: 'up' | 'down')  {  const  bounds  =  filterGroupBounds(rows,  index);  const  start  =  bounds?.start  ??  index;  const  end  =  bounds?.end  ??  index;  const  target  =  direction  ===  'up'  ?  start  -  1  :  end  +  1;  if  (target  <  0  ||  target  >=  rows.length  ||  (bounds  &&  ((direction  ===  'up'  &&  rows[target].id.startsWith('group-'))  ||  (direction  ===  'down'  &&  rows[target].id.startsWith('group-')))))  return  rows;  const  block  =  rows.slice(start,  end  +  1);  const  next  =  [...rows];  next.splice(start,  block.length);  const  insertAt  =  direction  ===  'up'  ?  start  -  1  :  start  +  1;  next.splice(insertAt,  0,  ...block);  return  next;  };
function  removeFilterRow<T extends { id: string }>(rows: T[], index: number)  {  const  bounds  =  filterGroupBounds(rows,  index);  if  (!bounds)  return  rows.filter((_,  rowIndex)  =>  rowIndex  !==  index);  const  innerCount  =  rows.slice(bounds.start  +  1,  bounds.end).filter((item)  =>  !item.id.startsWith('group-')).length;  const  hasChildGroup  =  rows.slice(bounds.start  +  1,  bounds.end).some((item)  =>  item.id.startsWith('group-open-nested-')  ||  item.id.startsWith('group-close-nested-'));  if  (innerCount  <=  1  ||  hasChildGroup  ||  rows[index].id.startsWith('group-open-')  ||  rows[index].id.startsWith('group-close-'))  return  rows.filter((_,  rowIndex)  =>  rowIndex  <  bounds.start  ||  rowIndex  >  bounds.end);  return  rows.filter((_,  rowIndex)  =>  rowIndex  !==  index);  };
