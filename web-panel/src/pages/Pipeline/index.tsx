...
const SqlEditor: React.FC<{ value: string; onChange: (value: string) => void; onRun: () => void; executing: boolean; suggestionItems: string[] }> = ({ value, onChange, onRun, executing, suggestionItems }) => {
  ...
  const lastToken = value.split(/\s+/).pop()?.toUpperCase() ?? '';
  const suggestions = lastToken ? keywords.filter((item) => item.startsWith(lastToken) && item !== lastToken) : [];
  ...
  const pickSuggestion = (item: string) => onChange(`${value.slice(0, value.length - lastToken.length)}${item} `);
  return <div className="flow-sql-editor"><pre aria-hidden="true">{highlighted}</pre><textarea value={value} onChange={(event) => onChange(event.target.value)} spellCheck={false} onKeyDown={(event) => { if (event.key === 'ArrowDown' && suggestions.length) { event.preventDefault(); setActiveSuggestion((current) => (current + 1) % suggestions.length); } else if (event.key === 'ArrowUp' && suggestions.length) { event.preventDefault(); setActiveSuggestion((current) => (current - 1 + suggestions.length) % suggestions.length); } else if (event.key === 'Enter' && suggestions.length && !event.ctrlKey && !event.metaKey) { event.preventDefault(); pickSuggestion(suggestions[activeSuggestion]); } else if (event.key === 'Enter' && (event.ctrlKey || event.metaKey)) { event.preventDefault(); onRun(); } }} />{suggestions.length > 0 && <div className="flow-sql-suggestions">{suggestions.slice(0, 8).map((item, index) => <button type="button" className={index === activeSuggestion ? 'is-active' : ''} key={item} onMouseDown={(event) => event.preventDefault()} onClick={() => pickSuggestion(item)}>{item}</button>)}</div>}<button type="button" className={`flow-sql-run ${executing ? 'is-paused' : ''}`} aria-label={executing ? 'Pause query' : 'Run query'} title={executing ? 'Pause query' : 'Run query'} onClick={onRun}>{executing ? <PauseOutlined /> : <CaretRightFilled />}</button></div>;
};
