...
const  DataLayersWorkspace:  React.FC  =  ()  =>  {
    ...
    const  dataRows  =  baseDataRows.map((row)  =>  ({  ...row,  ...(rowEdits[row.record_id]  ??  {})  })).filter((row)  =>  !deletedRowKeys.has(row.record_id)  &&  (partition  ===  undefined  ||  row.partition  ===  mockPartitions.findIndex((item)  =>  item.name  ===  partition)  %  2)  &&  (!filterText  ||  Object.values(row).join('  ').toLowerCase().includes(filterText.toLowerCase()))  &&  filterRules.every((rule)  =>  rule.active  !==  false  &&  (!rule.value  ||  String(row[rule.field  as  keyof  typeof  row]  ??  '').toLowerCase().includes(rule.value.toLowerCase())))).sort((a,  b)  =>  {  const  av  =  String(a[sortField  as  keyof  typeof  a]  ??  '');  const  bv  =  String(b[sortField  as  keyof  typeof  b]  ??  '');  return  (av.localeCompare(bv,  undefined,  {  numeric:  true  })  ||  0)  *  (sortDirection  ===  'asc'  ?  1  :  -1);  });
                    ...
                    <div  className="flow-inspector-body">
                        <div  className="flow-navicat-toolbar">
                            ...
                            {filterOpen  &&  <div  className="flow-filter-card">
                                ...
                                <div  className="flow-filter-sort"><strong>排序方式</strong><span  className="flow-filter-sort-field-wrap"><FilterFieldPicker  value={sortField}  onChange={setSortField}  className="flow-filter-sort-field"  /><Select  className="flow-filter-sort-direction"  size="small"  value={sortDirection}  onChange={setSortDirection}  options={[{  value:  'asc',  label:  <SortDirectionIcon  direction="asc"  />  },  {  value:  'desc',  label:  <SortDirectionIcon  direction="desc"  />  }]}  /><DownOutlined  className="flow-filter-sort-chevron"  /></span><button  type="button"  className="flow-filter-mini flow-filter-sort-add"  aria-label="Add sort field"  onClick={()  =>  setFilterRules((rows)  =>  {  const  next  =  {  id:  crypto.randomUUID(),  field:  'data_type',  operator:  '=',  value:  '',  active:  true,  join:  'and'  };  return  rows.length  ===  1  &&  rows[0].id === 'placeholder-rule'  ?  [next]  :  [...rows,  next];  })}><FilterAddIcon /></button></div>
                                <div  className="flow-filter-search"><Input  allowClear  size="small"  value={filterText}  onChange={(event)  =>  setFilterText(event.target.value)}  placeholder="搜索当前结果集"  /></div>
                            </div>}
